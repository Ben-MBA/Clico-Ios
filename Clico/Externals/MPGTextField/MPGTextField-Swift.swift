...
@objc protocol MPGTextFieldDelegate{
    ...
    @objc optional func textFieldDidEndEditing(_ textField: MPGTextField_Swift, withSelection data: Dictionary<String,AnyObject>)
...
class MPGTextField_Swift: UITextField, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate {
    ...
    @IBInspectable var mDelegate : MPGTextFieldDelegate?
    ...
    */
    ...
    func provideSuggestions(){
        if let tvc = self.tableViewController {
            ...
            var frameForPresentation = self.convert(bounds, to: superview!.superview!)
            frameForPresentation.origin.y += self.frame.size.height;
            frameForPresentation.size.height = 200;
...
//   MARK:- Filter Method
    ...
    func applyFilterWithSearchQuery(_ filter : String) -> [[String: AnyObject]]
    {
        let lower = (filter as NSString).lowercased
        let filteredData = data.filter({
                if let match : AnyObject  = $0["DisplayText"]{
                    return (match as! NSString).lowercased.hasPrefix((lower as NSString).lowercased)
                }
                else{
                    return false
                }
    ...
    func handleExit(){
        ...
        if ((mDelegate?.textFieldShouldSelect?(self)) != nil){
            if self.applyFilterWithSearchQuery(self.text!).count > 0 {
                ...
                mDelegate?.textFieldDidEndEditing?(self, withSelection: ["DisplayText":self.text! as AnyObject, "CustomObject":"NEW" as AnyObject])
            }
        }
